...
cossim_f = nn.CosineSimilarity(dim=1)
...
class BasicBlock(nn.Module):
    ...
    def __init__(
        ...
        reveal_internal: bool = False,  # for NeighborVariation detector
        topk: int = 1,  # for NeighborVariation detector
    ) -> None:
        super().__init__()
        ...
        self.stride = stride
        self.reveal_internal = reveal_internal
        self.topk = topk
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.reveal_internal:
            indices = find_nearest_neighbor(out, self.topk)
            nn_index.append(indices)
...
class ResNet(nn.Module):
    def __init__(
        ...
        norm_layer: Optional[Callable[..., nn.Module]] = None,
        reveal_internal: bool = False,  # for NeighborVariation
        topk: int = 1,
    ) -> None:
        super().__init__()
        ...
        for _ in range(1, blocks):
            layers.append(
                block(
                    ...
                    reveal_internal=self.reveal_internal,
                    topk=self.topk,
                )
            )
