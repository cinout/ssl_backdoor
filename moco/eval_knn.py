...
parser.add_argument(
    "--weights", dest="weights", type=str, help="pre-trained model weights"
)
parser.add_argument(
    "--load-cache",
    action="store_true",
...
parser.add_argument("--tsne", action="store_true", help="visualize tsne")
parser.add_argument(
    "--feature_loc", type=str, default="", help="location of tsne features"
)
...
def main_worker(args):
    ...
    # ---------------------------------------------------------------
    train_loader, val_loader, val_loader_poisoned = get_loaders(
    ...
    if args.load_cache and os.path.exists(cached_feats):
        ...
        train_feats, train_labels, train_index = get_feats(
            train_loader, model, args.print_freq
        )
    ...
    if args.load_cache and os.path.exists(cached_feats):
        ...
        val_feats_poisoned, val_labels_poisoned, val_index_poisoned = get_feats(
            val_loader_poisoned, model, args.print_freq, save_flag=True
        )
        torch.save(
            (val_feats_poisoned, val_labels_poisoned, val_index_poisoned), cached_feats
        )
    ...
    for k in [1, 50]:
        ...
        acc, conf_matrix_clean, acc_poisoned, conf_matrix_poisoned = faiss_knn(
            train_feats,
            train_labels,
            val_feats,
            val_labels,
        ...
        np.save(
            "{}/k_{}/conf_matrix_poisoned.npy".format(args.save, k),
...
def faiss_knn(
    ...
    conf_matrix_poisoned = np.zeros(
        (int(targets_val_poisoned.max()) + 1, int(targets_val_poisoned.max()) + 1)
    )
            ...
            if save_flag:
                ...
                if i < 10:
                    for batch_index in range(images.size(0)):
                        ...
                        inv_image1.save(
                            "{}/val_poisoned_images/".format(args.save)
                            + str(img_ctr).zfill(5)
                            + ".png"
                        )
