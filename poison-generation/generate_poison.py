...
def add_watermark(
    ...
    if text:  # watermark is a string
        ...
        if position == "center":
            ...
            for w in [int(base.size[0] * i) for i in [0.25, 0.5, 0.75]]:
                for h in [int(base.size[1] * i) for i in [0.25, 0.5, 0.75]]:
                    d.text(
                        (w, h), watermark, font=fnt, fill=(0, 0, 0, 128), anchor="mm"
                    )
        ...
        if alpha_composite:
            ...
            w_width, w_height = watermark_width, int(
                img_watermark.size[1] * watermark_width / img_watermark.size[0]
            )
            ...
            if position == "center":
                ...
                for w in [int(base_image.size[0] * i) for i in [0.25, 0.5, 0.75]]:
                    for h in [int(base_image.size[1] * i) for i in [0.25, 0.5, 0.75]]:
                        ...
                        # Halve all alpha values
                        location = (
                            max(0, min(location[0], na.shape[1])),
                            max(0, min(location[1], na.shape[0])),
                        )  # if location is negative, clip at 0
                        na[..., 3][
                            location[1] : (location[1] + w_height),
                            location[0] : (location[0] + w_width),
                        ] *= alpha
            ...
            if position == "center":
                ...
                location = (
                    random.randint(
                        int(base_image.size[0] * 0.25 - w_width / 2),
                        int(base_image.size[0] * 0.75 - w_width / 2),
                    ),
                    random.randint(
                        int(base_image.size[1] * 0.25 - w_height / 2),
                        int(base_image.size[1] * 0.75 - w_height / 2),
                    ),
                )
                transparent.paste(img_watermark, location, mask=img_watermark)
            else:
...
def generate_poison(
    ...
    for split in splits:
        if split == "train":
            if patch_params:
                train_filelist = "data/{}/train/loc_{}_loc-min_{:.2f}_loc-max_{:.2f}_alpha_{:.2f}_width_{}_rate_{:.2f}_targeted_{}_w-patch-parameters_filelist.txt".format(
                    experimentID,
                    train_location,
                    train_location_min,
                    train_location_max,
                ...
                train_filelist = "data/{}/train/loc_{}_loc-min_{:.2f}_loc-max_{:.2f}_alpha_{:.2f}_width_{}_rate_{:.2f}_targeted_{}_filelist.txt".format(
                    experimentID,
                    train_location,
                    train_location_min,
                    train_location_max,
        ...
        if split == "val_poisoned":
            val_poisoned_filelist = "data/{}/val_poisoned/loc_{}_loc-min_{:.2f}_loc-max_{:.2f}_alpha_{:.2f}_width_{}_filelist.txt".format(
                experimentID,
                val_location,
                val_location_min,
                val_location_max,
    ...
    if train_filelist:
        ...
        for file_id, file in enumerate(tqdm(train_filelist)):
            if file_id < len_poisoned:
                # add watermark
                poisoned_image, location, (w_width, w_height) = add_watermark(
                    file.split()[0],
                    trigger,
                    text=text,
