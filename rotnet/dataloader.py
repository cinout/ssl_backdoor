    ...
    def __init__(self, path_to_txt_file, transform=None):
        ...
        with open(path_to_txt_file, "r") as f:
            ...
            if "g" in myhost:
                self.file_list = [
                    row.replace(
                        "/home/aniruddha/", "/nfs/ada/hpirsiav/users/anisaha1/data/"
                    )
...
class GenericDataset(data.Dataset):
    def __init__(
        ...
        # ANIRUDDHA ImageNet100
        elif self.dataset_name == "imagenet100":
            assert (
                self.split == "train"
                or self.split == "val"
                or self.split == "val_poisoned"
            )
            self.mean_pix = [0.485, 0.456, 0.406]
            self.std_pix = [0.229, 0.224, 0.225]
            ...
            self.transform = transforms.Compose(transforms_list)
            self.data = Places205(
                root=_PLACES205_DATASET_DIR, split=self.split, transform=self.transform
            )
