...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        self.fc = nn.Sequential(
            Flatten(), nn.Linear(512 * block.expansion, num_classes)
        )
...
def resnet18(opt, pretrained=False, progress=True, **kwargs):
    ...
    """
    return _resnet(
        "resnet18", BasicBlock, [2, 2, 2, 2], pretrained, progress, opt, **kwargs
    )
...
if __name__ == "__main__":
    ...
    for f in range(len(out)):
        print(
            "Output feature {0} - size {1}".format(net.all_feat_names[f], out[f].size())
        )
