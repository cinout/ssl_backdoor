...
class Algorithm:
    ...
    def init_optimizer(self, net, optim_opts, key):
        ...
        if optim_type == "adam":
            ...
            optimizer = torch.optim.SGD(
                ...
                nesterov=(
                    optim_opts["nesterov"] if ("nesterov" in optim_opts) else False
                ),
    ...
    def solve(self, data_loader_train, data_loader_test, data_loader_p_test):
        ...
        for self.curr_epoch in range(start_epoch, self.max_num_epochs):
            ...
            if data_loader_test is not None:
                ...
                eval_stats = self.evaluate(data_loader_p_test)
                self.logger.info("==> Evaluation stats poisoned: %s" % (eval_stats))
                # self.keep_record_of_best_model(eval_stats, self.curr_epoch)
    ...
    def evaluate_conf_matrix(self, dloader):
        self.logger.info("Evaluating: %s" % os.path.basename(self.exp_dir))
        ...
        for idx, batch in enumerate(tqdm(dloader())):
            self.biter = idx
            eval_stats_this, pred_var, labels_var = self.evaluation_step_conf_matrix(
                batch
            )
            for layer_id in range(5):
                pred_var_stack[layer_id] = torch.cat(
                    (pred_var_stack[layer_id], pred_var[layer_id].cpu()), dim=0
                )
            labels_var_stack = torch.cat((labels_var_stack, labels_var.cpu()), dim=0)
            eval_stats.update(eval_stats_this)

        self.logger.info("==> Results: %s" % eval_stats.average())

        return eval_stats.average(), pred_var_stack, labels_var_stack
