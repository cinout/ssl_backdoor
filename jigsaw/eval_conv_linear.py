...
parser.add_argument(
    "--save", default="./output/distill_1", type=str, help="experiment output directory"
)
parser.add_argument(
    "-e",
    "--evaluate",
...
parser.add_argument(
    "--weights", type=str, required=True, help="pre-trained model weights"
)
parser.add_argument(
    "--lr_schedule", type=int, nargs="*", default=[15, 30, 40], help="lr drop schedule"
)
parser.add_argument(
...
class MultipleLinearLayers(nn.Module):
    def __init__(self, arch, nc):
        ...
        fmt_str = " {:>12s}"
        self.acc_prefixes = [fmt_str.format(layer_id) for layer_id in layer_ids]
    ...
    def forward(self, inputs):
        out_linear = [
            linear_layer(inp) for linear_layer, inp in zip(self.linear_layers, inputs)
        ]
        return out_linear
...
def main_worker(args):
    ...
    if args.evaluate:
        acc_meters, pred_var_stack_all, labels_var_stack = validate(
            val_loader, backbone, model, acc_prefixes, args
        )
        acc_p_meters, pred_var_p_stack_all, labels_var_p_stack = validate(
            val_p_loader, backbone, model, acc_prefixes, args
        )
        ...
        for layer_id in range(5):
            ...
            conf_matrix_clean = np.zeros(
                (int(labels_var_stack.max()) + 1, int(labels_var_stack.max()) + 1)
            )
            conf_matrix_poisoned = np.zeros(
                (int(labels_var_stack.max()) + 1, int(labels_var_stack.max()) + 1)
            )
            ...
            for i in range(pred_var_p_stack.size(0)):
                ...
                conf_matrix_poisoned[
                    int(labels_var_p_stack[i]), int(pred_var_p_stack[i])
                ] += 1
            ...
            np.save(
                "{}/conf_matrix_clean_layer_{}.npy".format(args.save, layer_id + 1),
                conf_matrix_clean,
            )
            np.save(
                "{}/conf_matrix_poisoned_layer_{}.npy".format(args.save, layer_id + 1),
                conf_matrix_poisoned,
            )

            with open(
                "{}/conf_matrix_layer_{}.csv".format(args.save, layer_id + 1), "w"
            ) as f:
                f.write("Model {},,Clean val,,,,Pois. val,,\n".format(""))
                f.write("Data {},,acc1,,,,acc1,,\n".format(""))
                f.write(
                    ",,{:4f},,,,{:4f},,\n".format(
                        acc_meters[layer_id].avg, acc_p_meters[layer_id].avg
                    )
                )
...
def train(train_loader, backbone, model, optimizer, acc_prefixes, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if not i:
            acc_meters = [
                NoBatchAverageMeter("", ":>11.2f") for i in range(len(outputs))
            ]
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            if not i:
                ...
                progress = NoTabProgressMeter(
                    len(val_loader), [batch_time, *acc_meters], prefix="Test: "
                )
            ...
            for layer_id in range(5):
                pred_var_stack[layer_id] = torch.cat(
                    (pred_var_stack[layer_id], outputs[layer_id].cpu()), dim=0
                )
